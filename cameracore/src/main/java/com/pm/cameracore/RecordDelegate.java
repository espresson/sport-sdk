...
//                    continue;
                ...
                mVideoSize = SizeUtils.chooseOutputSize(outputSizes, new Size(width, height), new Size(16, 9), false);
...
//                    .show(getChildFragmentManager(), FRAGMENT_DIALOG);
    ...
    public void startRecordingVideo(String SmartName) {
        Log.e("llllllllll",mCameraDevice+";;;"+mPreviewSize);
        if (null == mCameraDevice || null == mPreviewSize) {
            return;
        }
        closePreviewSession();
        setUpMediaRecorder(SmartName);
        SurfaceTexture texture = mCallback.getSurfaceTexture();
        assert texture != null;
        texture.setDefaultBufferSize(mPreviewSize.getWidth(), mPreviewSize.getHeight());
        try {
...
//        mNextVideoAbsolutePath = null;
    ...
    public final static String DATABASE_PATH = File.separator + "sdcard" + File.separator + "myshopt" + File.separator + "vedio" + File.separator;

    final SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss", Locale.US);
    File videoDir = new File(DATABASE_PATH);

    private String getVideoFilePath(Context context,String SmartName) {

        if (!videoDir.exists()) {
            videoDir.mkdirs();
        }

        return videoDir.getAbsolutePath() + File.separator +SmartName+ "_" + sdf.format(new Date()) + ".mp4";
    }
    public String getmNextVideoAbsolutePath() {
        return mNextVideoAbsolutePath == null ? "" : mNextVideoAbsolutePath;
    }
    ...
    private void setUpMediaRecorder(String SmartName) {
        if (null == mContext) {
            return;
        }
        ...
        if (mNextVideoAbsolutePath == null || mNextVideoAbsolutePath.isEmpty()) {
            mNextVideoAbsolutePath = getVideoFilePath(mContext,SmartName);
        }
